...
def main(cfg: FairseqConfig) -> None:
    ...
    cfg.__setattr__("EXP_DOMAIN", os.environ["EXP_DOMAIN"])
    if distributed_utils.is_master(cfg.distributed_training):
        with open(os.path.join(cfg.checkpoint.save_dir, "config.yaml"), "w") as f:
            f.write(OmegaConf.to_yaml(cfg))
        exp_id = os.path.basename(cfg.checkpoint.save_dir)
        eval_client = MQEvaluationClient(
    ...
    if not cfg.dataset.disable_validation:
        # logger.info('begin validation on "{}" subset'.format(valid_subsets))
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        # train for one epoch
        valid_losses, should_stop = train(cfg, trainer, task, epoch_itr, eval_client)
        if should_stop:
            break
...
def train(
    ...
    progress = progress_bar.progress_bar(
        ...
        wandb_tags=cfg.common.wandb_tags,
        azureml_logging=(
    ...
    for i, samples in enumerate(progress):
        ...
        if eval_client is not None and num_updates % 10 == 0:
            eval_result = eval_client.get_evaluation_result()
            if eval_result is not None:
                if hasattr(eval_result, "command") and eval_result.command == "stop":
                    print(f"command: {eval_result.command}")
                    should_stop = True
                else:
                    progress.log(
                        eval_result.results.__dict__
                        | {"eval_step": eval_result.eval_step},
                        tag="cp_result",
                        step=num_updates,
                    )
        if num_updates % 10 == 0 and distributed_utils.get_world_size(global_group) > 1:
            should_stop_tensor = torch.tensor([should_stop], dtype=torch.bool).cuda()
            distributed_utils.broadcast(
                tensor=should_stop_tensor,
                src=0,
                group=global_group,
            )
            should_stop = should_stop_tensor.cpu().item()

        end_of_epoch = not itr.has_next()
        valid_losses, _should_stop = validate_and_save(
            cfg, trainer, task, epoch_itr, valid_subsets, end_of_epoch
        )
        should_stop |= _should_stop
        if (
            valid_losses is not None
            and eval_client is not None
            and not cfg.common.no_evaluation
        ):
            print(f"request evaluation at {num_updates}")
            eval_client.request_evaluation(num_updates)
        if should_stop:
            if eval_client is not None:
                for eval_result in eval_client.wait4clean_up():
                    progress.log(
                        eval_result.results.__dict__
                        | {"eval_step": eval_result.eval_step},
                        tag="cp_result",
...
def validate_and_save(
    ...
    should_stop |= should_stop_early(
        cfg, valid_losses[0] if valid_losses is not None else None
    )
    ...
    if do_save or should_stop:
        cp_path = checkpoint_utils.save_checkpoint(
            ...
            trainer,
            epoch_itr,
            valid_losses[0] if valid_losses is not None else None,
        )
