...
@register_model("bfn_roberta")
class BFNRobertaModel(FairseqEncoderModel):
    @classmethod
    def hub_models(cls):
        return {
    ...
    def load_state_dict(self, state_dict, strict=True, **kwargs):
        ...
        for k, p in self.named_parameters():
            if k not in new_state_dict:
                logger.info("not loading {}".format(k))
                new_state_dict[k] = p
                un_initialized_keys.append(k)
        super().load_state_dict(new_state_dict, strict=strict, **kwargs)
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            metavar="D",
            default=64,
            help="number time dimensions, -1 means keeping all ffn blocks",
        )
    ...
    def build_model(cls, args, task):
        ...
        encoder = BFNRobertaEncoder(args, task.source_dictionary).cuda()
...
class MultiplyByConstant(nn.Module):
    def __init__(self, constant):
        ...
        # Ensure constant is registered as a parameter so it can be moved to the
        # correct device. However, requires_grad is False since we don't want to
        # update it during training.
        self.constant = nn.Parameter(torch.tensor(constant), requires_grad=True)

    def forward(self, x):
        return x * self.constant


class BFNInputLayer_bak(nn.Embedding):
    def __init__(self, time_dim, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._start, self._end = 0.0, 1.0
        self.time_dim = time_dim
        self.width = (
            ((self._end - self._start) / self.time_dim) if self.time_dim > 0 else 0
        )
        self.centers = None

        self.input_mapper = nn.Sequential(
            nn.Linear(self.num_embeddings + time_dim, self.embedding_dim),
        )
    ...
    def gaussian_basis(self, x):
        ...
        if self.centers is None:
            self.centers = torch.linspace(
                self._start, self._end, self.time_dim, dtype=x.dtype, device=x.device
            )  # [feature_num]
    ...
    def forward(self, t, src_tokens):
        # input:
        #   t: B
        #   src_tokens: B x T x V
        # self.weight (embeddings): V x C
        # output: B x T x C
        B, T, V = src_tokens.size()
        if self.time_dim > 1:
            t = self.gaussian_basis(t)  # B x N x time_dim
        else:
            t = t.unsqueeze(-1)  # B x T x 1
        if self.time_dim > 0:
            input_feat = torch.cat(
                [src_tokens * 2 - 1, t], dim=-1
            )  # B x T x (V + time_dim)
        else:
            input_feat = src_tokens * 2 - 1
        return self.input_mapper(input_feat)


class BFNInputLayer(nn.Embedding):
    def __init__(self, time_dim, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._start, self._end = 0.0, 1.0
        self.time_dim = time_dim
        ...
        self.time_feature_mapping = nn.Linear(
            self.time_dim, self.embedding_dim, bias=True
        )

        # self.input_mapper = nn.Sequential(
        #     MultiplyByConstant(constant=1.0),
    ...
    def forward(self, t, src_tokens):
        ...
        if self.time_dim > 1:
            ...
            t = t.unsqueeze(-1)  # B x N x 1
        _output = F.linear(src_tokens, self.weight.T) + self.time_feature_mapping(t)
        # output = self.input_mapper(_output)
        output = _output
        return output


class BFNRobertaEncoder(FairseqEncoder):
    """RoBERTa encoder."""
    ...
    def build_embedding(self, vocab_size, embedding_dim, padding_idx, time_dim=64):
        return BFNInputLayer(time_dim, vocab_size, embedding_dim, padding_idx)

    def build_encoder(self, args, dictionary, embed_tokens):
        encoder = BFNTransformerEncoder(args, dictionary, embed_tokens)
...
@register_model_architecture("bfn_roberta", "bfn_roberta")
...
@register_model_architecture("bfn_roberta", "bfn_roberta_prenorm")
...
@register_model_architecture("bfn_roberta", "bfn_roberta_base")
def p_roberta_base_architecture(args):
    p_base_architecture(args)


@register_model_architecture("bfn_roberta", "bfn_roberta_large")
...
@register_model_architecture("bfn_roberta", "bfn_xlm")
