...
def apply_rotary_pos_emb(x, cos, sin, aa_mask=None): # bsz * self.num_heads, tgt_len, self.head_dim
...
class RotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, aa_mask: Optional[Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        self._cos_cached, self._sin_cached = self._update_cos_sin_tables(k, seq_dimension=-2)

        return (
            apply_rotary_pos_emb(q, self._cos_cached, self._sin_cached, aa_mask),
            apply_rotary_pos_emb(k, self._cos_cached, self._sin_cached, aa_mask),
        )
...
class RotaryMultiheadAttentionWithBias(nn.Module):
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = RotaryMultiheadAttentionWithBias._append_prev_key_padding_mask(
        ...
        attn_weights = RotaryMultiheadAttentionWithBias.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz)
