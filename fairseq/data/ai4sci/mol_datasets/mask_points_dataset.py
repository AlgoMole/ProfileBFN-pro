...
class MaskPointsDataset(BaseWrapperDataset):
    def __init__(
        ...
        if random_token_prob > 0.0:
            ...
            for special_id in vocab_special_list:
                weights[special_id] = 0
            # weights[vocab.special_index()] = 0
            weights[(np.arange(len(self.vocab)) < mol_vocab_range[0]) + (np.arange(len(self.vocab)) > mol_vocab_range[1])] = 0
    ...
    def __getitem_cached__(self, epoch: int, index: int):
        ...
        with data_utils.numpy_seed(self.seed, epoch, index):
            ...
            new_coord[mask, :] += self.noise_f(num_mask, self.noise)
