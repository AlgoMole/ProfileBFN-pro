...
from .base import BFNBaseModel


@register_model
class BFNFoldseekMutationModel(BFNBaseModel):
    def __init__(self,
                 foldseek_path: str,
                 config_path: str,
        ...
        super().__init__(task="lm_ProteinGym", **kwargs)
    ...
    def forward(self, wild_type, seqs, mut_info, structure_content, structure_type, plddt):
        ...
        for i, (seq, info) in enumerate(zip(seqs, mut_info)):
            ...
            # print(f"ori_seq = {ori_seq}, mut_data = {mut_data} ins_seq = {ins_seq}")
        ...
        if len(ins_seqs) > 0:
            ...
            ins_probs = ins_outputs.softmax(dim=-1)
        
        # print(f"ori_seqs [{len(ori_seqs), len(ori_seqs[0])}] = {ori_seqs[:5]}")
        ori_inputs = self.tokenizer.batch_encode_plus(ori_seqs, return_tensors="pt", padding=True)   # [32, L]
        ori_inputs = {k: v.to(device) for k, v in ori_inputs.items()}
        # work through model
        ori_outputs = self.model(**ori_inputs)
        ori_probs = ori_outputs.softmax(dim=-1)
        # print(f"ori_probs [{ori_probs.shape}] = {ori_probs}")
        ...
        for i, data_list in enumerate(mut_data):
            ...
            for data in data_list:
                ...
                # print(f"VOCAB = {self.tokenizer.get_vocab()} ")
                # print(f"saprot_VOCAB = {self.saprot_tokenizer.get_vocab()} ")
                # print(f"ori_aa = {ori_aa}, foldseek_struc_vocab = {foldseek_struc_vocab}")

                ori_st = self.saprot_tokenizer.get_vocab()[ori_aa + foldseek_struc_vocab[0]]
                mut_st = self.saprot_tokenizer.get_vocab()[mut_aa + foldseek_struc_vocab[0]]
    ...
    def predict_mut(self, seq: str, mut_info: str) -> float:
        """
               ...
               """
        tokens = self.saprot_tokenizer.tokenize(seq)
        for single in mut_info.split(":"):
            pos = int(single[1:-1])
            tokens[pos - 1] = "#" + tokens[pos - 1][-1]
        
        mask_seq = " ".join(tokens)
        inputs = self.saprot_tokenizer(mask_seq, return_tensors="pt")
