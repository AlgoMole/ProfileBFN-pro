...
@register_model
class BFNMutationModel(BFNBaseModel):
    def __init__(self,
    ...
    def forward(self, wild_type, seqs, mut_info, structure_content, structure_type, plddt):
        ...
        # print(f"nums of seqs in one set : {len(seqs)}")
        ...
        ori_probs = self.model(**ori_inputs)
        # ori_probs = ori_outputs.softmax(dim=-1)
        
        if self.MSA_log_path is not None:
            bfn_dict_array = ['<s>', '<pad>', '</s>', '<unk>', 'L', 'A', 'G', 'V', 'S', 'E', 'R', 'T', 'I', 'D', 'P', 'K', 'Q', 'N', 'F', 'Y', 'M', 'H', 'W', 'C', 'X', 'B', 'U', 'Z', 'O', '.', '-', '<null_1>', '<mask>']
            aa2id = {item:i for i, item in enumerate(bfn_dict_array) }
        ...
        for i, data_list in enumerate(mut_data):
            ...
            for data in data_list:
                ...
                if self.MSA_log_path is not None and st <= ori_pos -1 < ed and (ori_pos - 1 - st < MSA_log_prior.shape[0]):
