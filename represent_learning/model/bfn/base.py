...
from model.bfn.bfn_model import BFNForSequenceClassification
from model.bfn.bfn_model import BFN4ProtModel


class BFNBaseModel(AbstractModel):
    """
    ...
    def __init__(self,
        ...
        """
        assert task in ['classification', 'regression', 'lm', 'lm_ProteinGym', 'base']
        self.task = task
    ...
    def initialize_model(self):
        ...
        print(f"net name = {self.net_name}", flush=True)
        ...
        if self.task == 'classification':
            self.model = BFNForSequenceClassification(
            ...
            self.model = BFN4ProtModel.from_pretrained(self.net_name, net_override=self.extra_config)
        
        elif self.task == 'lm_ProteinGym':
            self.model = BFN4ProtModel.from_pretrained(self.net_name, net_override=self.extra_config)
            self.saprot_tokenizer = EsmTokenizer.from_pretrained(self.config_path)
        ...
        if self.freeze_backbone:
            if hasattr(self.model, "dplm"):
                for param in self.model.dplm.net.parameters():
                    param.requires_grad = False
            elif hasattr(self.model, "bfn"):
                for param in self.model.bfn.net.parameters():
                    param.requires_grad = False
            else:
    ...
    def get_hidden_states(self, inputs, reduction: str = None) -> list:
        ...
        if hasattr(self.model, "dplm"):
            outputs = self.model.dplm(inputs['input_ids'], return_last_hidden_state=True)
        elif hasattr(self.model, "bfn"):
            outputs = self.model.bfn(inputs['input_ids'], return_last_hidden_state=True)
        else:
