...
def infer_maxsep(train_data, test_data, report_metrics = False, 
    ...
    device = torch.device("cuda:0" if use_cuda else "cpu")
    dtype = torch.float32
    id_ec_train, ec_id_dict_train = get_ec_id_dict('./data/CLEAN/' + train_data + '.csv')
    id_ec_test, _ = get_ec_id_dict('./results/' + test_data + '.csv')
    # load checkpoints
    # NOTE: change this to LayerNormNet(512, 256, device, dtype) 
    # and rebuild with [python build.py install]
    # if inferencing on model trained with supconH loss
    ...
    if pretrained:
        try:
            checkpoint = torch.load('./checkpoints/CLEAN/'+ train_data +'.pth', map_location=device)
        except FileNotFoundError as error:
            raise Exception('No pretrained weights for this training data')
    else:
        try:
    ...
    # load precomputed EC cluster center embeddings if possible
    if train_data == "split70":
        emb_train = torch.load('./checkpoints/CLEAN/70.pt', map_location=device)
    elif train_data == "split100":
        emb_train = torch.load('./checkpoints/CLEAN/100.pt', map_location=device)
    else:
        emb_train = model(esm_embedding(ec_id_dict_train, device, dtype))
    ...
    if report_metrics:
        ...
        true_label, all_label = get_true_labels('./results/' + test_data.replace("_clean", ""))
        pre, rec, f1, roc, acc = get_eval_metrics(
            pred_label, pred_probs, true_label, all_label)
