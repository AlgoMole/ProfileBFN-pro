...
def retrive_esm1b_embedding(fasta_name):
    esm_script = "test/esm/scripts/extract.py"
    esm_out = "data/CLEAN/esm_data"
    esm_type = "esm1b_t33_650M_UR50S"
    fasta_name = "results/" + fasta_name + ".a3m"
...
def compute_esm_distance(train_file):
    ...
    _, ec_id_dict = get_ec_id_dict('./results/' + train_file + '.csv')
...
def prepare_infer_fasta(fasta_name):
    ...
    csvfile = open('./results/' + fasta_name +'.csv', 'w', newline='')
    csvwriter = csv.writer(csvfile, delimiter = '\t')
    csvwriter.writerow(['Entry', 'EC number', 'Sequence'])
    fastafile = open('./results/' + fasta_name +'.a3m', 'r')
...
def mask_sequences(single_id, csv_name, fasta_name) :
    csv_file = open('./results/'+ csv_name + '.csv')
    csvreader = csv.reader(csv_file, delimiter = '\t')
    output_fasta = open('./results/' + fasta_name + '.a3m','w')
...
def mutate_single_seq_ECs(train_file):
    id_ec, ec_id =  get_ec_id_dict('./results/' + train_file + '.csv')
    single_ec = set()
    ...
    for id in id_ec.keys():
        for ec in id_ec[id]:
            if ec in single_ec and not os.path.exists('./checkpoints/CLEAN/esm_data/' + id + '_1.pt'):
                single_id.add(id)
                break
